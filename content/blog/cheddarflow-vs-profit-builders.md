...
Profit Builders publishes its full data methodology at the [Results page](/results) — OPRA tape ingest, sweep detection per CBOE Rule 6.11, Black-Scholes-Merton Greeks, NBBO aggression classification. Every signal is stamped with `scorer_version` so the methodology is auditable across grader iterations. No cherry-picking, no deleted methodology revisions.
...
Open the [Profit Builders free scanner](/free-scanner) tomorrow at 9:30 AM ET, filter to Grade A only, and watch what happens by 10:30. If the methodology earns its keep, the 7-day trial unlocks real-time data, GEX, and the full history.
